...
namespace Ventas_Milton.Manejadores
{
    class Manejador_Mercaderia
    {
        ...
        public DataTable BuscarMercaderia(string parametro, csMercaderia mercaderia)
        {
            ...
            switch (opcion)
            {
                ...
                case 3: dt = dmercaderia.ConsultarMercaderiaXNombre(mercaderia);
                    break;
                case 4: dt = dmercaderia.ConsultarMercaderiaXCategoria(mercaderia);
                    break;
                ...
                //    break;
                case 15: dt = dmercaderia.ConsultarMercaderiaParaProducir(mercaderia);
                    break;
            }
            return dt;
        }
        ...
        public void actualizarStock(int id, double cantidad) {
            dMercaderia d = new dMercaderia();
            d.actualizarStock(id, cantidad);
        }

        public void Mercaderia_Ingresar(csMercaderia mercaderia)
        {
            dMercaderia dMerca = new dMercaderia();
            dMerca.Mercaderia_Ingresar(mercaderia);
        }
        public void Mercaderia_Actualizar(csMercaderia mercaderia)
        {
            dMercaderia dMerca = new dMercaderia();
            dMerca.Mercaderia_Actualizar(mercaderia);
        }
        public void Mercaderia_Anular(csMercaderia mercaderia) 
        {
            dMercaderia dMerca = new dMercaderia();
            dMerca.Mercaderia_Anular(mercaderia);
        }
        public DataTable Mercaderia_ListarxIdCategoria(Int32 idCategoria)
        {
            DataTable dt = new DataTable();
            dMercaderia dmercaderia = new dMercaderia();
            dt = dmercaderia.Mercaderia_ListarxIdCategoria(idCategoria);
            return dt;
        }
