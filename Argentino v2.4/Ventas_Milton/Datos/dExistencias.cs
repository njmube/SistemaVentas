...
namespace Ventas_Milton.Datos
{
    class dExistencias
    {
        ...
        {
            string c = "SELECT m.idMercaderia 'idMercaderia', e.idExistencias 'idExistencia', e.`codInternoMercaderia` 'Codigo Barras', m.`descripcion` 'Producto',  m.`precioVenta` 'Precio', m.stockDisponible 'stock' FROM `existencias` e  inner JOIN `mercaderia` m  on e.`idMercaderia` = m.`idMercaderia` Where e.codInternoMercaderia Like concat('%','" + p + "', '%') and e.eliminado = 0";
            return funciones.QueryReturnDataTable(c);
        }

        public int ventaExistencia(int e, int dm, string em)
        {
            string c = "update existencias set idMovimiento = "+dm+", estadoMercaderia = '"+em+"' where idExistencias = "+e+" " ;
            return funciones.QueryReturnInt(c);
        }
    }
}
