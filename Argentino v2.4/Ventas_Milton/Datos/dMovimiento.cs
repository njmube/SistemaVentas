...
namespace Ventas_Milton.Datos
{
    class dMovimiento
    {
        ...
        public int ingresarCompra(csMovimiento m)
        {
            string sqlquery = "INSERT INTO movimiento(tipoMovimiento,fecha,hora,idPersona,eliminado,montoPago,numeroDocumento, subtotal, igv, total, idTipoDocumento)"
                            + " VALUES(@tipoMovimiento,@fecha,@hora,@idPersona,@eliminado,@montoPago,@numDoc, @subtotal, @igv, @total, @idTipoDocumento)";
            MySqlConnection cn = con.getConexion();
            cn.Open();
            ...
            try
            {
                ...
                cmd.Parameters.Add("@hora", m.Hora);
                cmd.Parameters.Add("@idPersona", m.IdPersona);
                cmd.Parameters.Add("@eliminado", 0);
                cmd.Parameters.Add("@montoPago", m.MontoPago);
                cmd.Parameters.Add("@numDoc", m.numDocumento);
                cmd.Parameters.Add("@subtotal", m.subtotal);
                cmd.Parameters.Add("@igv", m.igv);
                cmd.Parameters.Add("@total", m.total);
                cmd.Parameters.Add("@idTipoDocumento", m.idTipoDocumento);
                cmd.ExecuteNonQuery();
