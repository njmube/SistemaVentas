    ...
    public partial class frmVentas : Form
    {
        ...
        void guardar() {
            ...
            using (ts)
            {
                ...
                mov.TipoMovimiento = param_tipoMovimiento;
                mov.Fecha = dtpFechaEmision.Value.Date;
                //mov.Fecha = dtpFechaEmision.Value;
                mov.MontoPago = Math.Round(Convert.ToDouble(txtTotal.Text));
                mov.CodOfertas = codOfertaTotal;
                mov.IdPersona = param_idPersona;
                mov.UbicacionInicial = ubicacionI.ToString();
                mov.UbicacionFinal = ubicacionF.ToString();
                mov.TipoMovimiento = cboTipoDocumento.SelectedValue.ToString();
                mov.numDocumento = txtSerie.Text + "-" + txtNumero.Text;

                mov.IdMovimiento = mMovimientos.RegistrarNuevoMovimiento(mov);

                int nFilas = dgvProdcutos.Rows.Count;
                ...
                foreach (DataGridViewRow producto in dgvProdcutos.Rows ){
                    ...
                    dm.IdMercaderia = Convert.ToInt32( producto.Cells[0].Value);
                    dm.IdMovimiento = mov.IdMovimiento;
                    dm.Precio = Math.Round(Convert.ToDouble(producto.Cells[2].Value),2);
                    dm.Cantidad = Math.Round(Convert.ToDouble(producto.Cells[3].Value),2);
                    dm.Subtotal = Math.Round(Convert.ToDouble(producto.Cells[4].Value), 2);
                    dm.Descuento = Math.Round(Convert.ToDouble(producto.Cells[5].Value), 2);
                    dm.Total = Math.Round(Convert.ToDouble(producto.Cells[6].Value), 2);
                    dm.IdDetalleMovimiento = mdm.RegistrarNuevoDetalleMov(dm);
                    for (int i = 0; i < nFilas; i++)
                    {
                        
                        int e_idM = Convert.ToInt32(dgvExistencias.Rows[i].Cells[0].Value);
                        if (e_idM == dm.IdMercaderia)
                        {
                            int e_idE = Convert.ToInt32(dgvExistencias.Rows[i].Cells[1].Value);
                            me.ventaExistencia(e_idE, dm.IdDetalleMovimiento, "VENDIDO");
                        }
                    }
                    double cant = Math.Round(Convert.ToDouble(producto.Cells[7].Value),2);
                    cant = cant - dm.Cantidad;
                    mM.actualizarStock(dm.IdMercaderia, cant);                   
                }
                ts.Complete();
                inicio();
                dgvExistencias.DataSource = null;
                dgvProdcutos.DataSource = null;
                dgvExistencias.Rows.Clear();
                dgvProdcutos.Rows.Clear();
            }
        }
        ...
        private void txtCodBarras_TextChanged(object sender, EventArgs e)
        {
            ...
            this.lvProductos.Columns[5].Width = 1;
